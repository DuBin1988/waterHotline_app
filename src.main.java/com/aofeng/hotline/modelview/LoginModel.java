	...
	public Command Login = new Command() {
		...
		public void Invoke(View view, Object... args) {
			...
			Thread th = new Thread(new Runnable() {
				...
				public void run() {
					try {
//						String sql = "select id, cname,  (select stationname from t_repair_station where id=station_id), (select stationid from t_repair_station where id=station_id) from t_pad_user where name='"
//								+ Name.get() + "' and pass='" + Password.get() +"'";
						String sql = "select ID, NAME, ENAME, charindex(roles,(select id from t_role where NAME = '维修人员'),1) from t_user where ename='"
								+ Name.get()
								+ "' and password='"
								+ Password.get() + "'";
						String urlString=Vault.DB_URL + "sql/" + URLEncoder.encode(sql).replace("+", "%20");
	...
	public Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			String USER_ID, CHECKER_NAME, STATION_NAME, STATION_ID;
			if (1 == msg.what) {
				...
				//如果登录名不同
				if(!Util.getSharedPreference(mContext, Vault.PASSWORD).equals(Password.get()))
				{
					// 保存用户信息
					...
					String s=CHECKER_NAME;
					String s1=Name.get();
					prefEditor.putString(Vault.USER_ID, USER_ID);
					prefEditor.putString(Vault.PASSWORD, Password.get());
					prefEditor.putString(Vault.STATION_NAME, STATION_NAME);				
					prefEditor.putString(Vault.STATION_ID, STATION_ID);				
					prefEditor.commit();
					//终止后台服务?阻塞操作？
					Util.stopServiceIfRunning(mContext, AlarmService.class);
				}
			...
			else if(-1 == msg.what)
			{
				...
				upgradeDatabase();
				// 页面跳转
				Intent intent = new Intent(mContext, MechanicsChooserActivity.class);

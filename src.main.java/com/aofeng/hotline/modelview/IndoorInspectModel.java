...
import com.aofeng.hotline.activity.AutographActivity;
import com.aofeng.hotline.activity.IndoorInspectActivity;
...
public class IndoorInspectModel {
	...
	public String[][] names = {
			...
			{"f_heater_overdue", "f_heater_softconnector", "f_heater_leakage", "f_heater_emit", "f_heater_trapped", "f_heater_leakage_valve", "f_heater_leakage_connetor", "f_heater_leakage_heater"},//热水器
			{"f_furnace_overdue", "f_furnace_softconnector", "f_furnace_leakage", "f_furnace_emit", "f_furnace_trapped",  "f_furnace_leakage_valve", "f_furnace_leakage_connetor", "f_furnace_leakage_furnace"},//壁挂炉
			{"f_precaution_kitchen", "f_precaution_openkitchen",  "f_precaution_multisource"}   //其他隐患
	};
	...
	public String[][] cnames={
			...
			{"超期使用","软管连接","漏气","直排","烟道未排出室外","球阀","接口","机身"},//热水器
			{"超期使用","软管连接","漏气","直排","烟道未排出室外","球阀","接口","机身"},//壁挂炉
			{"封闭式厨房","开放式厨房","多种火源"}//其他隐患
	};
...
//	
	...
	public ArrayList<Pair> getAllSuggestionItems()
	{
		Object[][] objs = {
				...
				{this.f_heater_overdue, this.f_heater_softconnector, this.f_heater_leakage, this.f_heater_emit, this.f_heater_trapped,  this.f_heater_leakage_valve, this.f_heater_leakage_connetor, this.f_heater_leakage_heater},//热水器
				{this.f_furnace_overdue, this.f_furnace_softconnector, this.f_furnace_leakage, this.f_furnace_emit, this.f_furnace_trapped,  this.f_furnace_leakage_valve, this.f_furnace_leakage_connetor, this.f_furnace_leakage_furnace},//壁挂炉
				{this.f_precaution_kitchen, this.f_precaution_openkitchen, this.f_precaution_multisource}   //其他隐患
		};
	...
	private void FillBindingList() {
		Object[] list = {this.f_property, this.f_iszhongdian,this.f_gongnuan,this.f_zuzhu,this.REGION_NAME,this.f_baojingqichang,this.f_biaochang,this.f_kachangjia,this.f_biaoqianfa,
				...
				this.f_bglweizhi,this.f_bglxianzhuang, this.f_jpg_type,
		...
		String[] codes = {"f_property", "f_iszhongdian","f_gongnuan","f_zuzhu","REGION_NAME","f_baojingqichang","f_biaochang","f_kachangjia","f_biaoqianfa",
				...
				"f_bglweizhi","f_bglxianzhuang", "f_jpg_type",
...
//	}
	...
	public void muteOthers(int paneId) {
		int[] panes = {R.id.basicPane, R.id.meterPane, R.id.plumPane, R.id.cookerPane, R.id.precautionPane, R.id.feedbackPane};//, R.id.furnacePane, R.id.suggestionPane、、壁挂锅炉布局
...
//	};
	...
	public Command firstpageButton = new Command() {
		@Override
		public void Invoke(View view, Object... args) {
			muteOthers(R.id.basicPane);
		}
	};
	public Command secondpageButton = new Command() {
		@Override
		public void Invoke(View view, Object... args) {
			muteOthers(R.id.meterPane);
		}
	};
	public Command thirdpageButton = new Command() {
		@Override
		public void Invoke(View view, Object... args) {
			muteOthers(R.id.plumPane);
		}
	};
	public Command fourthpageButton = new Command() {
		@Override
		public void Invoke(View view, Object... args) {
			muteOthers(R.id.cookerPane);
		}
	};
	public Command fifthpageButton = new Command() {
		@Override
		public void Invoke(View view, Object... args) {
			muteOthers(R.id.precautionPane);
		}
	};
	public Command sixthpageButton = new Command() {
		@Override
		public void Invoke(View view, Object... args) {
			muteOthers(R.id.feedbackPane);
		}
	...
	public Command ShowPurchaseHistory = new Command() {
		@Override
		public void Invoke(View view, Object... args) {
			if( (f_kahao.get() == null || f_kahao.get().length()==0) && (f_buygas.get()==null || f_buygas.get().length()==0))
			{
				Toast.makeText(mContext, "请先进行卡号查询。", Toast.LENGTH_SHORT).show();
				return;
			}
//			// 利用包袱传递参数给Activity
//			Bundle bundle = new Bundle();
//			bundle.putString("CARD_ID", f_kahao.get());
...
//			mContext.startActivity(intent);				
	...
	public Command BGLShiYongCmd = new Command() {
		...
		public void Invoke(View view, Object... args) {
			if(f_bglshiyong.get())
			{
				...
				IndoorInspectModel.this.f_furnace_emit.set(false);	
	...
	public Command RSQShiYongCmd = new Command() {
		...
		public void Invoke(View view, Object... args) {
			if(f_rshqshiyong.get())
			{
				...
				IndoorInspectModel.this.f_heater_emit.set(false);	
	 ...
	 * 如果未用胶皮管
	 */
	public Command jpgWuCmd = new Command() {
		@Override
		public void Invoke(View view, Object... args) {
			if(f_jpgwu.get())
			{
				((Spinner)mContext.findViewById(R.id.f_jpg_type)).setSelection(0);
				IndoorInspectModel.this.f_jpgzhengchang.set(false);
				IndoorInspectModel.this.f_jpglouqi.set(false);	
				IndoorInspectModel.this.f_jpglaohua.set(false);	
				IndoorInspectModel.this.f_jpgguochang.set(false);	
				IndoorInspectModel.this.f_jpgwuguanjia.set(false);	
				IndoorInspectModel.this.f_jpgdiaoding.set(false);	
				IndoorInspectModel.this.f_jpganmai.set(false);	
			}
		}
	};
	...
	private boolean NeedsRepair() {
		...
		if( f_sibiao.get() || f_changtong.get()|| f_fanzhuang.get()||f_qblouqi.get()||f_reading_mismatch.get()||f_qbqita.get()
				...
				|| f_heater_overdue.get()||f_heater_softconnector.get()||f_heater_emit.get()||f_heater_trapped.get()||f_heater_leakage.get()
				|| f_furnace_overdue.get()||f_furnace_softconnector.get()||f_furnace_emit.get()||f_furnace_trapped.get()||f_furnace_leakage.get()
				|| f_precaution_kitchen.get()||f_precaution_openkitchen.get()||f_precaution_multisource.get() || f_zgbeizhu.get().trim().length()>0
	...
	public Command SearchByICCardNo = new Command() {
		...
		public void Invoke(View view, Object... args) {
			...
			Thread th = new Thread(new Runnable() {
				...
				public void run() {
					try {
//						String sql = "SELECT user_name, user_link, city, f_area, region_name,JB_TABLE_CODE 基表号,table_spot 左右表, JB_FACTORY 基表厂家, " +
//"ROAD, UNIT_NAME, CUS_DOM, CUS_DY, CUS_FLOOR, CUS_ROOM, tsum, tcount from (select * from T_IC_USERFILE WHERE	CARD_ID = '" + IndoorInspectModel.this.f_kahao.get() +  "') t1 left join "  +
//"(SELECT CARD_ID, SUM (SELLGAS_GAS) tsum, count(sellgas_gas) tcount FROM	T_IC_SELLGAS	WHERE	CARD_ID = '" + IndoorInspectModel.this.f_kahao.get() + "' group by CARD_ID) t2 on  t1.card_id= t2.card_id";
//						String url = Vault.TUNNEL_URL + "sql%7C" 
//								+ URLEncoder
//								.encode(sql, "UTF8")
//										.replace("+", "%20");
//						HttpGet getMethod = new HttpGet(url);
						String sql ="SELECT f_username, f_phone, f_city, f_area, '',f_meternumber 基表号,f_aroundmeter 左右表, f_jbfactory 基表厂家, " +
								"f_road, f_districtname, f_cusDom, f_cusDy, f_cusFloor, f_apartment, tsum, tcount from (select * from t_userfiles WHERE f_cardid = '" + IndoorInspectModel.this.f_kahao.get() + "') t1 left join " +
								"(SELECT f_cardid, SUM (f_pregas) tsum, count(f_pregas) tcount FROM t_sellinggas WHERE f_cardid = '"+ IndoorInspectModel.this.f_kahao.get() +"' group by f_cardid) t2 on  t1.f_cardid= t2.f_cardid";
			...
			if (1 == msg.what) {
				...
				try {
					...
					if (!obj.has("col0")) {
						...
						//用户名
						if(obj.has("col0"))
							f_consumername.set(obj.getString("col0"));
						//电话
						if(obj.has("col1"))
							f_consumerphone.set(obj.getString("col1"));
						//市
						if(obj.has("col2"))
							city.set(obj.getString("col2"));
						//区
						if(obj.has("col3"))
							f_area.set(obj.getString("col3"));
						//网格
						if(obj.has("col4"))
						{
							int idx = REGION_NAME.indexOf(obj.getString("col4"));
							if(idx>=0)
								((Spinner)mContext.findViewById(R.id.REGION_NAME)).setSelection(idx);
						}
						//表编号
						if(obj.has("col5"))
							f_biaohao.set(obj.getString("col5"));
						//左右表
						if(obj.has("col6"))
						{
							int idx = f_rqbiaoxing.indexOf(obj.getString("col6"));
							if(idx>=0)
						...
						if(obj.has("col7"))
						{
							int idx = f_biaochang.indexOf(obj.getString("col7"));
							if(idx>=0)
								((Spinner)mContext.findViewById(R.id.f_biaochang)).setSelection(idx);
						}
						//总购气量
						if(obj.has("col14"))
							f_buygas.set(obj.getString("col14"));
						String archiveAddress = "-";
						for(int i=8; i<14; i++)
							if(obj.has("col" + i))
								archiveAddress += obj.getString("col"+i) + "-";
							else
								archiveAddress += "-";
						if(archiveAddress.endsWith("-"))
							archiveAddress = archiveAddress.substring(0, archiveAddress.length()-3);
						f_archiveaddress.set(archiveAddress);

						//街道
						if(obj.has("col8"))
							ROAD2.set(obj.getString("col8"));
						//街道
						if(obj.has("col9"))
							UNIT_NAME2.set(obj.getString("col9"));
						//街道
						if(obj.has("col10"))
							CUS_DOM2.set(obj.getString("col10"));
						//街道
						if(obj.has("col11"))
							CUS_DY2.set(obj.getString("col11"));
						//街道
						if(obj.has("col12"))
							CUS_FLOOR2.set(obj.getString("col12"));
						//街道
						if(obj.has("col13"))
							CUS_ROOM2.set(obj.getString("col13"));
					}
				} catch (Exception e) {
					e.printStackTrace();
					// 查不到此IC卡用户
	...
	private void Upload(boolean trueUpload) {
		...
		/*boolean isExistCheckPlan = CheckPlan(ROAD.toString(), UNIT_NAME.toString(), CUS_DOM.toString(), CUS_DY.toString(), CUS_FLOOR.toString(), CUS_ROOM.toString());
		if(isExistCheckPlan != false)
		{*/
			if(f_jujian.get())
				needsRepair = false;
			// 找所有的图片
...
//				poster.execute(imgs.toArray(new String[imgs.size()]));
	 ...
	 */
	/*
	private boolean CheckPlan(String ROAD, String UNIT_NAME, String CUS_DOM, String CUS_DY, String CUS_FLOOR, String CUS_ROOM) {
		SQLiteDatabase db = mContext.openOrCreateDatabase("safecheck.db",
				Context.MODE_PRIVATE, null);
		Cursor c = db.rawQuery(
				"SELECT id FROM T_IC_SAFECHECK_PAPAER "
						+ "where ROAD=? and UNIT_NAME=? and CUS_DOM=? and CUS_DY=? and CUS_FLOOR=? and CUS_ROOM=?", new String[] { ROAD, UNIT_NAME, CUS_DOM, CUS_DY, CUS_FLOOR, CUS_ROOM });
...
//	@RegexMatch(Pattern = "[0-9]{10}$", ErrorMessage = "卡号必须为10位数字！")
	...
	@Required(ErrorMessage = "IC卡基表读数不能为空")
	@RegexMatch(Pattern = "[0-9]*$", ErrorMessage = "IC卡基表读数必须为数字！")
	...
	public BooleanObservable f_lgzhengchang = new BooleanObservable(false);
	...
	public BooleanObservable f_bhgzhengchang = new BooleanObservable(false);
	...
	public BooleanObservable f_jpgzhengchang = new BooleanObservable(false);
	...
	public BooleanObservable f_jpgwu = new BooleanObservable(false);
	public ArrayListObservable<String> f_jpg_type = new ArrayListObservable<String>(String.class);
	...
	public BooleanObservable f_precaution_openkitchen = new BooleanObservable(false);
	...
	public ArrayListObservable<String> f_checktype = new ArrayListObservable<String>(String.class, new String[]{"安检单"});
	//小区性质
	...
	public BooleanObservable f_heater_emit= new BooleanObservable(false);
	...
	public BooleanObservable f_furnace_emit= new BooleanObservable(false);
...
//	public ArrayListObservable<RepairMan> RepairManList = new ArrayListObservable<RepairMan>(RepairMan.class);
	public StringObservable slipId = new StringObservable("");
	public StringObservable repairCode = new StringObservable("");
